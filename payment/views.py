...
def checkout(request):
    ...
    total_orders = cart.get_total()
    if request.user.is_authenticated:
        shipping_user = ShippingAddress.objects.get(user__id = request.user.id)
        shipping_form = ShippingForm(request.POST or None, instance=shipping_user)
        return render(request, 'payment/checkout.html', {'cart_products':cart_products, 'quantities':quantities, 'total_orders':total_orders, 'shipping_form':shipping_form})
...
def confirm_order(request):
    if request.POST:
        ...
        return render(request, 'payment/confirm_order.html', {'cart_products':cart_products, 'quantities':quantities, 'total_orders':total_orders, 'shipping_info':user_shipping})
...
def process_order(request):
    if request.POST:
        ...
        if request.user.is_authenticated:
            ...
            ord = get_object_or_404(Order, id=new_order.pk)
            for product in cart_products:
                prod = get_object_or_404(Product, id=product.id)
